...
mod sql_ast_traversal {
    ...
    pub trait SqlAstTraverser<Error> {
        ...
        fn traverse_ast_query(&mut self, query: &Box<ast::Query>)
            -> Result<Box<ast::Query>, Error>;
    ...
    impl SqlAstTraverser<String> for PathConvertor {
        ...
        fn traverse_statement(&mut self, statement: Statement) -> Result<Statement, String> {
            match statement {
                ...
                Statement::Truncate { .. } => {
                    Err("not implemented: Statement::Truncate".to_string())
                }
                ...
                Statement::AlterView { .. } => {
                    Err("not implemented: Statement::AlterView".to_string())
                }
                Statement::AlterRole { .. } => {
                    Err("not implemented: Statement::AlterRole".to_string())
                }
                ...
                Statement::SetNames { .. } => {
                    Err("not implemented: Statement::SetNames".to_string())
                }
                ...
                Statement::Rollback { .. } => {
                    Err("not implemented: Statement::Rollback".to_string())
                }
                ...
                Statement::Savepoint { .. } => {
                    Err("not implemented: Statement::Savepoint".to_string())
                }
                ...
                Statement::UnlockTables => {
                    Err("not implemented: Statement::UnlockTables".to_string())
                }
                Statement::Directory { .. } => {
                    Err("not implemented: Statement::Directory".to_string())
                }
            ...
            match drop {
                Statement::Drop {
                    ...
                    match object_type {
                        ObjectType::Table => {}
                        ObjectType::View => {
                            Err("not implemented: Statement::Drop::View".to_string())?
                        }
                        ObjectType::Index => {
                        ...
                        ObjectType::Role => {
                            Err("not implemented: Statement::Drop::Role".to_string())?
                        }
                        ObjectType::Sequence => {
        ...
        fn traverse_join_constraint(
            ...
            match constraint {
                ...
                JoinConstraint::Using(_) => {
                    Err("not implemented: JoinConstraint::Using".to_string())
                }
            }
        }
        ...
        fn traverse_table_factor(&mut self, relation: &TableFactor) -> Result<TableFactor, String> {
            match relation {
                ...
                TableFactor::UNNEST { .. } => {
                    Err("not implemented: TableFactor::UNNEST".to_string())
                }
                ...
                TableFactor::Unpivot { .. } => {
                    Err("not implemented: TableFactor::Unpivot".to_string())
                }
            }
        }
        ...
        fn traverse_select_item(&mut self, select_item: &SelectItem) -> Result<SelectItem, String> {
            match select_item {
                SelectItem::UnnamedExpr(expr) => {
                    Ok(SelectItem::UnnamedExpr(self.traverse_expr(expr)?))
                }
        ...
        fn traverse_expr(&mut self, expr: &Expr) -> Result<Expr, String> {
            match expr {
                ...
                Expr::IsDistinctFrom(_, _) => {
                    Err("Unhandled syntax: Expr::IsDistinctFrom".to_string())
                }
                ...
                Expr::MatchAgainst { .. } => {
                    Err("Unhandled syntax: Expr::MatchAgainst".to_string())
                }
        ...
        fn traverse_function_arg(
            ...
            match function_arg {
                ...
                FunctionArg::Named { .. } => {
                    Err("Unhandled syntax: FunctionArg::Named".to_string())
                }
            }
        }
        ...
        fn traverse_value(&mut self, value: &Value) -> Result<Value, String> {
            match value {
                ...
                Value::UnQuotedString(_) => {
                    Err("Unhandled value: Value::UnQuotedString".to_string())
                }
            }
        }
    }
    ...
    fn add_to_referencable_tables(
        ...
        match (table_name, alias) {
            ...
            (Some(table_name), None) => {
                ...
                referencable_tables
                    .insert(path, get_qualified_values_table_identifiers(&db_reference));
            }
            (None, None) => (),
        };
